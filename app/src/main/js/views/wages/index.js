    ...
    const dataList =  copyData.map((item,index)=>{
      //注意：所有负值的原值都是正数，需要相应改为负数
        ...
        if(item['是否转正'] == '否'){  //试用期
          ...
          item['应发工资'] = this.round(item['试用薪资'] + item['考勤扣除'] + item['其他扣除'] + item['工龄工资'],2)
        }
        
        else if(item['是否转正'] == '是'){ //转正后
          item['考勤扣除'] = -this.round(item['转正薪资']/zrts *(item['入职离职缺勤天数']+item['事假天数'])+ item['考勤扣费'],2)
          item['应发工资'] = this.round(item['转正薪资'] + item['考勤扣除'] + item['其他扣除'] + item['工龄工资'],2)
        }
          ...
          item['应发工资'] = this.round(item['试用薪资'] + item['转正差额']  + item['考勤扣除'] + item['其他扣除'] + item['工龄工资'],2)
        }
        /**社保扣除算法 */
        if(item['社保缴纳'] == '是'){
          if(item['是否城镇'] == '是'){
        ...
        /**缴纳个税算法 */
        if(jlgs){
          let balance = item['应发工资'] + item['社保扣除'] + item['公积金'] - 3500
          let tax = 0
